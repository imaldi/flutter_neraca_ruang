...
@RoutePage()
class KabarPage extends StatefulWidget {
...
class _KabarPageState extends State<KabarPage> {
  @override
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<DashboardCubit, DashboardState>(
      builder: (context, state) {
        var kabarTerbaru = state.listContentKabar ?? <Datum>[];
        // var kotaName = ref.watch(kotaNameProvider);
        var tagName = state.tags_name;
        var iconUrl = state.icon ?? "";
        var appbarTitle =
            // kotaName.isNotEmpty
            //     ? kotaName
            //     :
                    ...
                    child: Column(
                      children: [
                        ListView.builder(
                            ...
                            itemCount: kabarTerbaru.length ?? 0,
                            itemBuilder: (c, i) {
                              return ContentWidget(kabarTerbaru[i] ?? Datum());
                            }),
                        InkWell(
                          onTap: () {
                            context
                                ...
                                .fetchContentsWithCustomFilter(
                                    tagsId: state.tags_id,
                                    kotaId: state.kota_id,
                                    tipe: state.tipe ?? "",
                                    limit: state.limit ?? 0 + 3);
